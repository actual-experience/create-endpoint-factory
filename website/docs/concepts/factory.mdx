...
:::warning
There's no way to change `extra`'s type dynamically - if a property is optionally included, it will still be marked as potentially undefined even if the options guarantees it exists.
