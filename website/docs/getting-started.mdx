...
The combined handler receives data about the request and an API for responses, and automatically calls the corresponding handler - if no handler is found for the method, a `405` is sent along with an `Allow` header set to a list of supported methods.
