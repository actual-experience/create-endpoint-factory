...
```ts title="Full example, using Zod for schemas and Sequelize for database interaction"
...
export declare function getBookFromReq(req: NextApiRequest): Book | undefined;
...
import { createEndpointFactory } from '@actual/next-create-endpoint-handler';
...
import { alwaysMatch } from '@actual/next-create-endpoint-handler';
